...
Go 1.26 リリース連載の 3 本目です。

本記事では Go 1.26 で追加された Goroutine Leak Profiles について紹介します。

## アップデートの概要

Goroutine のリークを検出するためのプロファイルが [`runtime/pprof`](https://pkg.go.dev/runtime/pprof@go1.26rc2) に追加されました。
これまでは Uber 社が公開している [goleak](https://github.com/uber-go/goleak) などを用いてリークを検出するのが一般的なプラクティスでしたが、Go 標準のプロファイル機能を利用して検知ができるようになりました。
...
func leak() {
	...
	// 何かしらの処理でエラーが発生した場合、受信が行われずゴルーチンがリークする
...
func leak() {
	...
	// 何かしらの処理でエラーが発生した場合、送信が行われずゴルーチンがリークする
	if err := something(); err != nil {
		return
	}
...
リーク判定のために GC サイクルを走らせるため、本番環境における頻繁な取得は避け、手元検証やオンデマンド調査に使うのがよさそうです。
...
ここまで見るとテストの実行前後で Goroutine スタックを比較することでリークを検出する `goleak` のアプローチとは根本的に異なることがわかります。
...
ここまでみてきた通り、Go 1.26 で追加された `goroutineleak` プロファイルは、なかなか追いづらかった Goroutine リークを検知できる標準機能です。
Go 1.26 時点では実験的機能という位置付けですが、Go 1.27 でデフォルト有効になるのが待ち遠しいですね。
